...
void CSMain(uint3 dispatchThreadID : SV_DispatchThreadID)
{
	...
	float3 up = readPixel(dispatchThreadID.x, dispatchThreadID.y + 2);
	float3 left = readPixel(dispatchThreadID.x - 1, dispatchThreadID.y);
	float3 right = readPixel(dispatchThreadID.x + 1, dispatchThreadID.y);
	float3 down = readPixel(dispatchThreadID.x, dispatchThreadID.y - 2);

	pixel = (1.0 + 4.0 * SHARPEN_FACTOR) * pixel - SHARPEN_FACTOR * (up + left + right + down);
